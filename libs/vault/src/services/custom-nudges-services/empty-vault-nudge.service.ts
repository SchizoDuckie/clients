...
export class EmptyVaultNudgeService extends DefaultSingleNudgeService {
  ...
  shouldShowNudge$(nudgeType: VaultNudgeType, userId: UserId): Observable<NudgeStatus> {
    return combineLatest([
      ...
      switchMap(([status, ciphers, orgs, collections]) => {
        if (orgs == null || orgs.length === 0) {
          return status ? of(false) : of(ciphers == null || ciphers.length === 0);
        }
        ...
        if (status || managedCollections.length === 0 || canCreateCollections.length === 0) {
          return of(false);
        }
